    ...
    return (
        <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl shadow-lg w-full overflow-hidden">
            ...
            <div className="p-2 sm:p-4 flex flex-col items-center text-center">
                {/* Avatar */}
                <div className={`${overlay ? 'w-12 h-12' : 'w-16 h-16 sm:w-45 sm:h-45'} rounded-full border-5 p-0.5 flex items-center justify-center mb-3 ${isBatting ? 'border-sky-500/60 bg-sky-950/30' : 'border-emerald-500/60 bg-emerald-950/30'}`}>
                ...
                {/* Type label + Name */}
                <h4 className={`${overlay ? 'text-sm' : 'text-xs'} font-bold uppercase tracking-wider mb-1 ${isBatting ? 'text-sky-400' : 'text-emerald-400'}`}>
                    {type}: <span className={`font-heading text-white font-extrabold ${overlay ? 'text-2xl' : 'text-base sm:text-lg'} normal-case`}>{name}</span>
                </h4>

                {/* Stats row */}
                <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 mt-1">
                    {statItems.map((item, i) => (
                        <span key={i} className={`stat-value text-slate-400 ${overlay ? 'text-base' : 'text-xs'} whitespace-nowrap`}>{item}</span>
                    ))}
                </div>
                ...
                {isBatting && (todayStats || stats !== 'Sin datos aún') && (
                    <div className="mt-3 pt-2 border-t border-slate-800/60 w-full">
                        <span className={`stat-label ${overlay ? 'text-sm' : 'text-[10px]'}`}>HOY: </span>
                        <span className={`stat-value ${overlay ? 'text-base' : 'text-xs'} text-slate-300`}>{todayStats ?? stats}</span>
                    </div>
                )}
            </div>
        </div>
